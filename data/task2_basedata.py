...
class BaseData(object):
    ...
    def transform_wave(self, y, sr, mod):
        """
        波形を変形する
        """
        if self.augment:
            ...
            if self.aug_wcut > 0:
                ...
                if mod < 1000:
                    sf.write(
                        'mod/section_00_source_train_normal_' + str(mod + 1000) + '_mod' + '.wav', y,
                        samplerate=16000)

                if 1000 <= mod < 1003:
                    sf.write('mod/section_00_target_train_normal_' + '000' + str(
                        mod - 1000 + 3) + '_mod' + '.wav', y, samplerate=16000)

                if 1003 <= mod < 2003:
                    sf.write(
                        'mod/section_01_source_train_normal_' + str(mod - 3) + '_mod' + '.wav', y,
                        samplerate=16000)

                if 2003 <= mod < 2006:
                    sf.write('mod/section_01_target_train_normal_' + '000' + str(
                        mod - 2000) + '_mod' + '.wav', y, samplerate=16000)

                if 2006 <= mod < 3006:
                    sf.write(
                        'mod/section_02_source_train_normal_' + str(mod - 1006) + '_mod' + '.wav', y,
                        samplerate=16000)

                if 3006 <= mod < 3009:
                    sf.write('mod/section_02_target_train_normal_' + '000' + str(
                        mod - 3003) + '_mod' + '.wav', y, samplerate=16000)
    ...
    def get_features(self, files, desc, n_mels, n_frames, n_hop_frames, n_fft,
        ...
        for i, file in tqdm(enumerate(files), desc=desc):
            mod += 1
            features = self.file_to_features(file, n_mels, n_frames, n_fft, hop_length, power, mod)
    ...
    def file_to_features(self, wav_file, n_mels, n_frames, n_fft, hop_length, power, mod):
        """
        ...
        y, sr = self.transform_wave(y, sr, mod)
        # メルスペクトログラムを取得する
